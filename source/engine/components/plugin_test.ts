...
Deno.test(t(import.meta, "`.supported()` is a noop when supported"), { permissions: "none" }, async () => {
  await expect(Plugin.run({
    context: { id: import.meta.url, entity: "user", fatal: true },
  })).to.be.eventually.be.ok
})

Deno.test(t(import.meta, "`.supported()` throws when unsupported"), { permissions: "none" }, async () => {
  await expect(Plugin.run({
    context: { id: import.meta.url, entity: "organization", fatal: true },
  })).to.be.rejectedWith(MetricsError, /not supported for organization/i)
})
...
Deno.test(t(import.meta, "`.render()` renders template content"), { permissions: { read: [dir.source] } }, async () => {
  ...
  })).to.be.eventually.containSubset({ result: { content: "foo" } })
  await expect(Plugin.run({
    context: {
      id: import.meta.url,
      entity: "user",
      template: "metrics://engine/components/tests/template.ejs",
      fatal: true,
    },
  })).to.be.eventually.containSubset({
    result: { content: "foo" },
  })
})
...
Deno.test(
  t(
    import.meta,
    "`static .load()` without identifier returns the one defined by `Plugin.nameless`",
  ),
  { permissions: "none" },
  async () => {
    await expect(Plugin.load({ logs: "none" } as test))
      .to.eventually.be.instanceOf(Plugin)
      .and.to.have.property("id", Plugin.nameless)
...
for (const id of await Plugin.list()) {
  ...
  for (const test of tests) {
    for (const template of templates) {
      Deno.test(
        t(name, `*${template}* ${test.name}`),
        await getPermissions(test),
        async () => {
          const { teardown } = setup(test)
          await expect(process(
            deepMerge(
              { presets: { default: { plugins: { template } } } },
              deepMerge(config, test, { arrays: "replace" }),
            ),
          )).to.be.fulfilled.and.eventually.be.ok
          teardown()
        },
      )
    }
