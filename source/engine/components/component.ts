...
import { extractYaml as extract, test as hasfm } from "@std/front-matter"
...
export abstract class Component extends Internal {
  ...
  protected async run(state: state) {
    ...
    for (
      let attempt = 1;
      attempt <= (this.context.retries?.attempts ?? 1);
      attempt++
    ) {
      if (attempt > 1) {
        this.log.message(
          `attempt ${attempt} of ${this.context.retries.attempts}`,
        )
      }
      try {
        ...
        if (
          ...
          error[MetricsError.unrecoverable as keyof MetricsError]
        ) {
          recoverable = false
        }
      }
      ...
      if (error) {
        ...
        if (
          ...
          attempt < this.context.retries.attempts
        ) {
  ...
  async tests() {
    ...
    return YAML.parse(content) as Array<
      is.infer<typeof config> & { name: string }
    >
  }
    ...
    return list(`${path}/*.md`, { sync: true }).map((file) => {
      ...
      if (hasfm(raw)) {
        const {
          ...
          body: content,
        } = extract<{ type: string; title: string }>(raw)
  ...
  static async load(context: Record<PropertyKey, unknown> & { id: string }) {
    let error = null
    const url = /^(https?|file):/.test(context.id) ? new URL(context.id) : context.id.startsWith("metrics://") ? toFileUrl(context.id.replace("metrics:/", dir.source)) : toFileUrl(
      `${this.path}/${context.id}${context.id.endsWith("_test.ts") ? "" : "/mod.ts"}`,
    )
  ...
  static async run({
    ...
    state: state
    context: Record<PropertyKey, unknown> & { id: string }
  }) {
